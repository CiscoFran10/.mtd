...
function Cards() {
	...
	return (
		<section className="flex flex-col-reverse items-center justify-center w-full px-4 pt-8 bg-no-repeat bg-cover shadow md:flex-col md:gap-10 md:items-end md:min-h-screen bg-desktop md:pl-6 md:px-0 md:pt-0 md:animate-[bg-position_1.5s_ease] transition duration-300">
			<div className="card -mt-16 md:-mt-0  bg-card-front mr-[10%] md:mr-12 lg:mr-20 p-5 md:p-8 tracking-widest -translate-x-40 animate-[fade-in_1s_ease_forwards_0.3s] opacity-0">
				...
				<div className="flex items-center justify-between w-full mt-3 md:mt-5 text-xs md:text-sm lg:text-base lg:mt-7 text-gray-violet-100 ">
					...
					<h3> {`${normalizeMonth || "00"}/${data?.cc_year || "00"}`}</h3>
				</div>
			</div>

			<div className="card bg-card-back tracking-widest translate-x-40 animate-[fade-in_1s_ease_forwards_0.3s] opacity-0">
				<div className="h-full w-full flex justify-end items-center">
					<h3 className="block text-sm mr-10 sm:text-base lg:mr-14">
						{data?.cc_cvc || "000"}
					</h3>
				</div>
			</div>
